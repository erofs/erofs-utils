 ...
 * Copyright (C) 2016 Gao Xiang <xiang@kernel.org>
 *
 * Use SHA256 instead of the original MD5 to remove redundant
 * dependencies.  -- Gao Xiang
 */
#ifdef __linux__
#define _BSD_SOURCE
...
#define CS_SIZE		sizeof(((sum_t *)0)->out)
...
char *getln(char *buf, int size, FILE *fp)
{
	char *p;
	int l;
...
void sum_init(sum_t *cs)
{
	erofs_sha256_init(&cs->sha256);
}

void sum_fini(sum_t *cs)
{
	erofs_sha256_done(&cs->sha256, cs->out);
}

void sum_add(sum_t *cs, void *buf, unsigned int size)
{
	erofs_sha256_process(&cs->sha256, buf, size);
}

void sum_add_sum(sum_t *dst, sum_t *src)
{
	sum_add(dst, src->out, sizeof(src->out));
}

void sum_add_u64(sum_t *dst, uint64_t val)
{
	uint64_t v = htobe64(val);
	sum_add(dst, &v, sizeof(v));
}

void sum_add_time(sum_t *dst, time_t t)
{
	sum_add_u64(dst, t);
}

char *sum_to_string(sum_t *dst)
{
	int i;
...
int namecmp(const void *aa, const void *bb)
{
	char * const *a = aa;
	char * const *b = bb;
...
#ifdef HAVE_SYS_XATTR_H
int sum_xattrs(int fd, sum_t *dst)
{
	ssize_t buflen;
	ssize_t len;
	char *buf;
...
int sum_file_data_permissive(int fd, sum_t *dst)
{
	int ret;
...
int sum_file_data_strict(int fd, sum_t *dst)
{
	int ret;
	off_t pos;
...
void excess_file(const char *fn)
{
	printf("only in local fs: %s\n", fn);
}

void missing_file(const char *fn)
{
...
void check_match(char *fn, char *local_m, char *remote_m,
...
void check_manifest(char *fn, char *m, char *c, int last_call)
{
	char *rem_m;
	char *rem_c;
	char *l;
